...
> Looking for sharing `localStorage`-synchronized state across multiple components? You can also check out [`createLocalStorageState`](/create-local-storage-state).
