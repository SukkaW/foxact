...
> Looking for a state-management with a similar API, that can persisting state into `localStorage` or `sessionStorage`? Check out [`createLocalStorageState`](/create-local-storage-state) and [`createSessionStorageState`](/create-session-storage-state).

> Looking for a reducer-based context state management? Check out [`createContextReducer`](/context-reducer).
...
// By retutning an array, `createContextState` allows you to name your provider and hooks as you like:
...
```tsx filename="s../../components/sidebar.tsx" copy
import { memo } from 'react';
...
```tsx filename="s../../components/navbar.tsx" copy
import { memo } from 'react';
...
export default memo(function MainLayout({ children }: React.PropsWithChildren) {
  return (
    <SidebarActiveProvider initialState={true}>
      {children}
    </SidebarActiveProvider>
  );
});
...
export const [SidebarActiveProvider, useSidebarActive, useSetSidebarActive, SidebarActiveContext] = createContextState(false);
```

```tsx filename="../../components/sidebar.tsx" copy
...
export default memo(function Sidebar({ loggedIn }: SidebarProps) {
