...
> Looking for sharing `sessionStorage`-synchronized state across multiple components? You can also check out [`createSessionStorageState`](/create-session-storage-state).
