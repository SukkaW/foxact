...
```tsx filename="src/context/provider.tsx" copy
...
Global state management libraries (like [Redux](https://redux.js.org/) or [Zustand](https://github.com/pmndrs/zustand)) usually store the state outside of React, then use `useSyncExternalStore` to sync the state into the React.
